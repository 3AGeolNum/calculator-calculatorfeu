...
bool start_with_operator(std::string &input_from_user, int &size_of_string){
    ...
    if(size_of_string == 0){
        std::cout << "After removing the '+' at the beginning your calculation is empty : please correct it" << std::endl;
        return false;
    }
    ...
    for (int char_index = 0; char_index < size_of_string-1; ++char_index){
        ...
        && (next_char == multiply ||  next_char == divide || next_char == pluss)){
           ...
           return false;
            ...
            std::cout << "You put 2 dots side by side : please correct this" << std::endl;
            return false;
        }
    }
    return true;
...
void replace_non_operator_minus(std::string &input_from_user, int &size_of_string){
    ...
    //Should be done before the checking of 2 following operators because it can then happen at the beginning
    ...
    for (int char_index = 0; char_index < size_of_string-1; ++char_index){
        ...
        char actual_char = input_from_user[char_index];
        char next_char = input_from_user[char_index +1];
        if((actual_char == minuss || actual_char == pluss || actual_char == divide || actual_char == multiply) && next_char == minuss){
...
void add_zero_before_alone_point(std::string &input_from_user, int &size_of_string){

    //The input can not have 2 following operators, it has no spaces, no comma, it is not empty,
    //it does not start or end with an operator and all characters are valid
    ...
    for (int char_index = 0; char_index < size_of_string-1; ++char_index){
        char actual_char = input_from_user[char_index];
        char next_char = input_from_user[char_index +1];
        if (( actual_char == multiply || actual_char == divide || actual_char == pluss || actual_char == minuss || actual_char == other_minus)
        && (next_char == '.')){
...
void add_zero_after_alone_point(std::string &input_from_user, int &size_of_string){
    ...
    for (int char_index = 0; char_index < size_of_string-1; ++char_index){
        char actual_char = input_from_user[char_index];
        char next_char = input_from_user[char_index +1];
        if (( next_char == multiply || next_char == divide || next_char == pluss || next_char == minuss)
        && (actual_char == '.')){
            input_from_user.insert(char_index +1, "0");
            size_of_string++;
            }
